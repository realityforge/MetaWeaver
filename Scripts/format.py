...
try:
    index_files = subprocess.check_output(["git", "diff-index", "--cached", "--name-only", "--diff-filter=d", "HEAD", "--", *args.files],
    ...
    for file in files:
        ...
        elif file.lower().endswith(".h") or file.lower().endswith(".cpp") or file.lower().endswith(".cs") or file.lower().endswith(".md") or file.lower().endswith(".uplugin"):
    ...
    files_to_format_assuming_text = list(set(files_to_format_assuming_text))
    files_to_format_assuming_text.sort()
    ...
    if args.dry_run:
        ...
        if 0 != len(files_to_format):
            temp_file = make_filelist(files_to_format)
            result = subprocess.run(["clang-format", "--dry-run", "--Werror", "-i", f"--files={temp_file}"])
        ...
        for file in files_to_format_assuming_text:
            remove_bom(file)
            normalize_line_endings(file)

        if 0 != len(files_to_format):
            temp_file = make_filelist(files_to_format)
            subprocess.run(["clang-format", "-i", f"--files={temp_file}"], check=True)
    ...
    if args.verbose:
        if 0 != len(files_to_format_assuming_text):
            print("Processed the following files as TEXT:")
            for file in files_to_format_assuming_text:
                print(file)
