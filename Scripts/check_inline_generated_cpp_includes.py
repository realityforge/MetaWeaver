...
Usage:
  python3 Scripts/check_inline_generated_cpp_includes.py [--root <path>]
...
def list_cpp_files(source_root: Path) -> List[Path]:
    return list(source_root.rglob("*.cpp"))


def cpp_contains_macro_for(text: str, base: str) -> bool:
    pattern = re.compile(r"^\s*#\s*include\s+UE_INLINE_GENERATED_CPP_BY_NAME\(\s*" + re.escape(base) + r"\s*\)\s*$", re.MULTILINE)
...
def macro_is_after_all_other_includes(text: str, base: str) -> bool:
    ...
    if not includes:
        return False
    macro_line_re = re.compile(r"^\s*#\s*include\s+UE_INLINE_GENERATED_CPP_BY_NAME\(\s*" + re.escape(base) + r"\s*\)\s*$")
    idx_macro = None
