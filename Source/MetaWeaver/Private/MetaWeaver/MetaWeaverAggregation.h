...
inline void GatherIncludedSetsRecursive(
    UMetaWeaverMetadataDefinitionSet *Root,
  ...
  if (Root && !Visited.Contains(Root)) {
    ...
    for (const auto &SoftIncluded : Root->MetadataDefinitionSets) {
      if (const auto Included = SoftIncluded.LoadSynchronous()) {
        ...
        UE_LOG(LogMetaWeaver, Error,
               TEXT("Failed to load referenced set %s at Index=%d in asset %s"),
               *SoftIncluded.ToString(), Index, *Root->GetPathName());
      }
      Index++;
    }
...
inline void FlattenActiveSets(
    const TArray<TSoftObjectPtr<UMetaWeaverMetadataDefinitionSet>> &ActiveSets,
    TArray<UMetaWeaverMetadataDefinitionSet *> &OutSets) {
  OutSets.Reset();
  TSet<const UMetaWeaverMetadataDefinitionSet *> Visited;
...
inline void BuildEffectiveSpecsForClass(
    const UClass *Class,
